...
class Xls_writer(parse):  # we inherit the class from Avito_parse3
    ...
    def page(self):
        ...
        self.pages = int(input('Напишите, сколько страниц спарсить, или напишите 0, тогда по умолчанию спарсятся все страницы : '))
        print()
    ...
    def parser_1(self):
        ...
        if exists('parsed.xlsx') == False:
            ...
            self.name = 'parsed.xlsx'
            self.count = 0
            while exists(self.name)== True:

                self.count +=1
                self.name = f'{self.name}_{self.count}.xlsx'
            self.workbook = xlsxwriter.Workbook(f'parsed_{self.name}.xlsx')
